...
def d2_train_model(train_json_path,train_images_dir,val_json_path,val_images_dir,ims_per_batch, model_lr, bach_size_per_img, max_train_iter,num_workers,num_labels):
    ...
    new_root = os.path.join(work_root,str(model_name)+"_%s_%s_%s_%s"%(str(model_lr),str(bach_size_per_img),str(max_train_iter),str(ims_per_batch)))
    ...
    if os.path.exists(model_path):
        ...
        imgs_list = [os.path.join(test_dir,file_name)   for file_name in os.listdir(test_dir) if file_name.endswith(".jpg") or file_name.endswith(".png") or file_name.endswith(".bmp")]
...
if __name__ == "__main__":
    ...
    model_lr = 0.0025
    bach_size_per_img = 256
    max_train_iter = 10#5000
    ims_per_batch = 6
    num_workers = 2
    num_labels = 2

    # train_json_path = sys.argv[1]
    # train_images_dir = sys.argv[2]
    # val_json_path = sys.argv[3]
    # val_images_dir = sys.argv[4]
    ...
    d2_train_model(train_json_path,train_images_dir,val_json_path,val_images_dir,ims_per_batch, model_lr, bach_size_per_img, max_train_iter,num_workers,num_labels)
